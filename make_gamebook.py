...
#this code based on this divider. The font "f3" is used throughout the prolog because the default
#font used by LibreOffice when generating RTF documents is "f3 (Liberation Serif)". This way,
#the Ubuntu font overrides any remaining RTF commands within the "interactive_rtf" string.
with open(file_name[:-4] + " Gamebook.rtf", "w") as g:
    if file_name[-4:] == ".txt":
        g.write(r"{\rtf1 \ansi \deff3 {\fonttbl {\f3 Ubuntu;}} \f3 \fs24 \qj \par " +
        interactive_rtf.replace("\par\par\pard", "\par\par") + "}")
    #The 43rd character in each section will be "}" in an RTF document generated from LibreOffice.
    #In a RTF file generated from LibreOffice, the RTF command "{\loch" immediately precedes the "** Section"
    ...
    #(as opposed to having been generated by Python in TintypeText).
        ...
        g.write(r"{\rtf1 \ansi \deff3 {\fonttbl {\f3 Ubuntu;}} \f3 \fs24 \qj \par " +
        interactive_rtf.replace("\par\par\pard", "\par") + "}")
    elif file_name[-4:] == ".rtf" and interactive_rtf[42] == "}":
        g.write(r"{\rtf1 \ansi \deff3 {\fonttbl {\f3 Ubuntu;}} \f3 \fs24 \qj \par {" +
        ...
        interactive_rtf.replace("\par\par\pard", "\par\par").replace("\par \pard\plain ", "")
        .replace("fs20", "fs24").replace("\ql", "\qj") + "}")
